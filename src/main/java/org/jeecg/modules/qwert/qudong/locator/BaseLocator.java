...
abstract public class BaseLocator<T> {
     ...
     * @param slaveId a int.
     * @param offset a int.
     * @return a {@link org.jeecg.modules.qwert.qudong.locator.BaseLocator} object.
     */
    ...
    public static BaseLocator<Number> holdingRegister(int slaveId, int offset, int dataType) {
        return new NumericLocator(slaveId, RegisterRange.DIANZONG_REGISTER, offset, dataType);
    }
    ...
    public static BaseLocator<Boolean> holdingRegisterBit(int slaveId, int offset, int bit) {
        return new BinaryLocator(slaveId, RegisterRange.DIANZONG_REGISTER, offset, bit);
    }
    ...
    protected void validate(int registerCount) {
        try {
            QwertUtils.validateOffset(offset);
            QwertUtils.validateEndOffset(offset + registerCount - 1);
        }
        catch (QudongTransportException e) {
            throw new QwertIdException(e);
        }
    }
