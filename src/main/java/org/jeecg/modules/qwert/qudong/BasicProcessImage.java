...
import org.jeecg.modules.qwert.qudong.base.RangeAndOffset;
import org.jeecg.modules.qwert.qudong.code.RegisterRange;
import org.jeecg.modules.qwert.qudong.exception.IllegalDataAddressException;
import org.jeecg.modules.qwert.qudong.exception.QwertIdException;
import org.jeecg.modules.qwert.qudong.locator.BaseLocator;
import org.jeecg.modules.qwert.qudong.locator.NumericLocator;
import org.jeecg.modules.qwert.qudong.locator.StringLocator;
...
public class BasicProcessImage implements ProcessImage {
    ...
    private final Map<Integer, Short> dianzongRegisters = new HashMap<>();
    ...
    public BasicProcessImage(int slaveId) {
        QwertUtils.validateSlaveId(slaveId, false);
    ...
    public void setBinary(int range, int offset, boolean value) {
        if (range == RegisterRange.COIL_STATUS)
            ...
            throw new QwertIdException("Invalid range to set binary: " + range);
    }
    ...
    public synchronized void setNumeric(int range, int offset, int dataType, Number value) {
        ...
        // Write the value.
        if (range == RegisterRange.DIANZONG_REGISTER)
            setDianzongRegister(offset, registers);
        else if (range == RegisterRange.INPUT_REGISTER)
            setInputRegister(offset, registers);
        else
            throw new QwertIdException("Invalid range to set register: " + range);
    }
    ...
    /**
     * <p>setDianzongRegister.</p>
     *
     * @param offset a int.
     * @param registers an array of {@link short} objects.
     */
    public synchronized void setDianzongRegister(int offset, short[] registers) {
        validateOffset(offset);
        for (int i = 0; i < registers.length; i++)
            setDianzongRegister(offset + i, registers[i]);
    }
    ...
    public synchronized void setHoldingRegisterBit(int offset, int bit, boolean value) {
        ...
        setDianzongRegister(offset, setBit(s, bit, value));
    }
     ...
     * @param bit a int.
     * @return a boolean.
     * @throws org.jeecg.modules.qwert.qudong.exception.IllegalDataAddressException if any.
     */
    public boolean getBit(int range, int offset, int bit) throws IllegalDataAddressException {
        ...
        throw new QwertIdException("Invalid range to get register: " + range);
    }
     ...
     * <p>getRegister.</p>
     *
     * @param locator a {@link org.jeecg.modules.qwert.qudong.locator.BaseLocator} object.
     * @param <T> a T object.
     * @return a T object.
     * @throws org.jeecg.modules.qwert.qudong.exception.IllegalDataAddressException if any.
     */
    public synchronized <T> T getRegister(BaseLocator<T> locator) throws IllegalDataAddressException {
        ...
        for (int i = 0; i < words; i++) {
            ...
            else if (locator.getRange() == RegisterRange.DIANZONG_REGISTER)
            ...
            b[i * 2] = QwertUtils.toByte(s, true);
            b[i * 2 + 1] = QwertUtils.toByte(s, false);
        }
    ...
    public synchronized short getHoldingRegister(int offset) throws IllegalDataAddressException {
        return getShort(offset, dianzongRegisters);
    }

    /** {@inheritDoc} */
    @Override
    public synchronized void setDianzongRegister(int offset, short value) {
        validateOffset(offset);
        dianzongRegisters.put(offset, value);
    }

    /** {@inheritDoc} */
    @Override
    public synchronized void writeHoldingRegister(int offset, short value) throws IllegalDataAddressException {
        short old = getShort(offset, dianzongRegisters);
        setDianzongRegister(offset, value);
    ...
    private void validateOffset(int offset) {
        if (offset < 0 || offset > 65535)
            throw new QwertIdException("Invalid offset: " + offset);
    }

    private void validateBit(int bit) {
        if (bit < 0 || bit > 15)
            throw new QwertIdException("Invalid bit: " + bit);
    }
