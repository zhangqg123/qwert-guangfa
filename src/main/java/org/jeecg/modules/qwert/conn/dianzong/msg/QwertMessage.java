...
import org.jeecg.modules.qwert.conn.dianzong.serial.dianzong.DianzongMessage;
...
abstract public class QwertMessage {
    ...
    final public void write2(ByteQueue queue,int r) {
        ...
        writeImpl2(qs);
		try {
			queue.push(toBytes(qs.toString().toUpperCase()));
			int s = queue.size();
			if(r==2) {
				byte[] rd = getRetData();
				queue.push(rd);
