...
public class JstZcTargetController extends JeecgController<JstZcTarget, IJstZcTargetService> {
	...
	@AutoLog(value = "jst_zc_target-分页列表查询")
	@ApiOperation(value="jst_zc_target-分页列表查询", notes="jst_zc_target-分页列表查询")
	@GetMapping(value = "/orgUserList")
	public Result<?> queryPageOrgUserList(JstZcTarget jstZcTarget,
								   @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
								   @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
								   HttpServletRequest req) {
		String orgUser = req.getParameter("orgUser");
		
		Result<Page<JstZcTarget>> result = new Result<Page<JstZcTarget>>();
		Page<JstZcTarget> pageList = new Page<JstZcTarget>(pageNo,pageSize);
		pageList = jstZcTargetService.queryJztPageByOrgUser(pageList, orgUser);
